...
for i1=-6:6%以病灶最明显层为中心取13张图片
    n1=n0+i1;
if ismember(p,[ 69,71,83,91,93,95,98,106,112,116,118])%ismember函数:查看后面矩阵中的数是不是前面矩阵中的成员
                fileform =[ 'H:\乳腺病例\分类\DCE\',num2str(p),'\*.dcm'];
filepathsrc = ['H:\乳腺病例\分类\DCE\',num2str(p),'\'];
file_outord = dir(fileform);%dir('*.m')：列出当前目录下符合正则表达式的文件夹和文件
c1=struct2cell(file_outord);
c2=c1{1,n1};
I = dicomread([filepathsrc,c2]);%读取图像数据
...
I =double(I);
m_fil=5;sigma=0.1;
H=fspecial('gaussian',[m_fil m_fil],sigma);%fspecial函数：建立用来预定义的滤波算子
