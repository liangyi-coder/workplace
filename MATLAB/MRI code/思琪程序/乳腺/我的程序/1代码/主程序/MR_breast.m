...
if ismember(p,[ 69,71,83,91,93,95,98,106,112,116,118])
 fileform =[ 'I:\classify\DCE\',num2str(p),'\*.dcm'];
filepathsrc = ['I:\classify\DCE\',num2str(p),'\'];
...
seq=1;
 n1=n1+k(p);
I = dicomread(['I:\classify\DCE\',num2str(p),'\',num2str(n1),'_',num2str((9*(n1-k(p)*seq-1))+(seq)),'.dcm']);
   I=imresize(imrotate(I,180),[1024,1024]);
else
    try
    I = dicomread(['I:\classify\DCE\',num2str(p),'\',num2str(n1),'_',num2str(n1-1),'.dcm']);%读取图像 
    catch
     I1 = dicomread(['I:\classify\MASK\',num2str(p),'\',num2str(n1),'_',num2str(n1-1),'.dcm']);%读取图像 
     I2 = dicomread(['I:\classify\Subtraction\',num2str(p),'\',num2str(n1),'_',num2str(n1-1),'.dcm']);%读取图像 
     I=I1+I2;
    end
end
[M,N]=size(I);
...
%保存整体乳腺（Ibreast），病灶侧（Ip），对侧乳腺区（In）
 in=Ibreast;
 save(['E:\乳腺\我的程序\1数据\MR\DCE\输入腺体\原始\5\in_mr',num2str(p)],'in');
 in=Ip;
 save(['E:\乳腺\我的程序\1数据\MR\DCE\输入腺体\原始\3\in_mr',num2str(p)],'in');
 in=In;
 save(['E:\乳腺\我的程序\1数据\MR\DCE\输入腺体\原始\4\in_mr',num2str(p)],'in');
